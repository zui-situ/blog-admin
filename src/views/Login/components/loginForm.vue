  ...
  <div class="form-container">
    <el-form v-if="showLogin" ref="loginFormRef" :model="loginForm" status-icon :hide-required-asterisk="true" :rules="rules" label-width="100px" class="login-form">
      <el-form-item label="账号" prop="username">
        <el-input v-model="loginForm.username" autocomplete="off" placeholder="请输入登录用户"></el-input>
      </el-form-item>
...
export default defineComponent({
  ...
  setup(_props, { emit }) {
    ...
    const rules = {
      ...
      username: [{ required: true, message: '请输入注册邮箱', trigger: 'change' }],
    ...
    const handleRegister = () => {
      registerRef.value.validate(async (valid: any) => {
        if (valid) {
          try {
            const { username, password, capcha } = state.registerForm
            const data = {
              username,
              capcha,
              // password
    ...
    const handleGetCaptcha = async (): Promise<boolean> => {
      try {
        const { username } = state.registerForm
        if (!username) {
          ElMessage({
            type: 'warning',
            message: '请输入注册邮箱'
