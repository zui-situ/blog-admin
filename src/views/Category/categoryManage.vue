<template>
  <div class="new">
    <el-card class="card-ctrl">
      <el-form :inline="true" :model="state.searchForm" class="demo-form-inline">
        <el-form-item label="类别名">
          <el-input v-model="state.searchForm.bindName" placeholder="类别名"></el-input>
        </el-form-item>
        <el-form-item label="类别状态">
          <el-select v-model="state.searchForm.bindStatus" placeholder="类别状态">
    ...
    <el-dialog v-model="addVisible" width="632px" title="新增类别">
      <CategoryAdd ref="categoryAddRef" @success="onAddSuccess"></CategoryAdd>
    </el-dialog>
  </div>
</template>
...
const handleEdit = (index: any, row: { _id: any }) => {
  addVisible.value = true
  nextTick(() => {
    categoryAddRef.value.getInfo(row._id)
  })
}
// 删除
const handleDelete = (row: any) => {
  Service.categoryRemove(row._id).then((res) => {
    if (res.code === 200) {
...
const editStatus = (row: any) => {
  Service.categoryEditStatus(row._id, { status: row.status === 1 ? 0 : 1 }).then((res) => {
    if (res.code === 200) {
      if (row.status === 0) {
